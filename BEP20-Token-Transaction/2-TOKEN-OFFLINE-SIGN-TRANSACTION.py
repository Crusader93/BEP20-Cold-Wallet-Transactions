...
def signTx():
        tx = open(os.path.join(sys.path[0], "UnsignedTokenTransaction.txt"), "r")
        signed_tx = web3.eth.account.signTransaction(ast.literal_eval(tx.read()),private_key)

        with open(os.path.join(sys.path[0], "SignedTokenTransaction.txt"), "w+") as f:
            f.write(str(signed_tx.rawTransaction))
            f.close()

        print("Transaction:")
        print(signed_tx.rawTransaction)
        print("Signed transaction was saved to the SignedTokenTransaction.txt file")
